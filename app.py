...
def show_daily_sales(data):
    ...
    with st.container(height=510):
        st.header(f"Melhores vendedores, {selected_date:%m/%d/%y}")
    ...
    with st.container(height=510):
        st.header(f"Piores vendedores, {selected_date:%m/%d/%y}")
...
def show_monthly_sales(data):
    ...
    with st.container(height=510):
        st.header(f"Vendas diárias para todos os produtos, {this_month:%B %Y}")
        monthly_sales = data[(data.index < next_month) & (data.index >= this_month)]
        st.write(monthly_sales)
    with st.container(height=510):
        st.header(f"Vendas totais de todos os produtos, {this_month:%B %Y}")
